...
export default class MainNavigationStickyPlugin extends window.PluginBaseClass {
    ...
    _toggleVisibility() {
        ...
        if (window.scrollY > this.scrollY) {
            if(!this.el.classList.contains(this.options.visibleCls)) {
                ...
                window.PluginManager.initializePlugin('FlyoutMenu', '[data-flyout-menu]');
                window.PluginManager.initializePlugin('RemoteClick', '[data-remote-click]');
            }
        } else {
            this.el.innerHTML = '';
